...
export class Basemap extends WebMapPlugin<{
  ...
  /** 底图项池 */
  private _basemapItemPool : Map<string, $Supermap.__ImageryLayer[]> = new Map()

  /** 当前选中的底图项Key值 */
  private _selectedKey : string

  /** 底图图层 */
  private _layerGroup: $Supermap.__ImageryLayer[] = []
  ...
  private _createTianDiTuItems () {
    ...
    const createTianDiTuItem = (name: string) => {
      this.createCustomBasemap(`天地图${name}`, new Supermap.UrlTemplateImageryProvider({
        url: this._tianDiTuUrls[`${name}底图`]
      }))
      this.createCustomBasemap(`天地图${name}含注记`, [
        new Supermap.UrlTemplateImageryProvider({ url: this._tianDiTuUrls[`${name}注记`] }),
        new Supermap.UrlTemplateImageryProvider({ url: this._tianDiTuUrls[`${name}底图`] }),
      ])
  ...
  createCustomBasemap (key: string, imageryProviders: $Supermap.__ImageryProvider | $Supermap.__ImageryProvider[]) : this {
    ...
    this._basemapItemPool.set(key, _imageryProviders.map(item => new Supermap.ImageryLayer(item), { $type: 'basemap-layer' }))
    return this
  }
