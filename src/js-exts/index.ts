...
export function ext (_this: Date) : IDataExtension
export function ext (_this: string) : IStringExtension
export function ext (_this: number) : INumberExtension
export function ext<T> (_this: T[]) : IArrayExtension<T>
export function ext<T> (
  ...arg:
  [Date] | [string] | [number] | [T[]]
) : IDataExtension | IStringExtension |INumberExtension | IArrayExtension<T> { // eslint-disable-line
  ...
  if (Array.isArray(_this)) {
    return {
      ...
      unique () {
        ext(_this as T[]).reset(...new Set(_this as T[]))
      },
      getUnique () {
